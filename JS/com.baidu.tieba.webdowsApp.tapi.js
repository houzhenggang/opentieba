 ...
 *   独立版：
 *      使用方法：
 *      浏览器中打开http://c.tieba.baidu.com/c/u/user/profile 然后按F12 然后按Console
 *      然后把这个文件所有内容复制，然后粘贴到那里，然后回车。
 *      然后就可以登录了：_s.tiebaApi.login("用户名","密码")
 */
...
(function (e, t) {
    ...
    }}, x.each({mouseenter: "mouseenter", mouseleave: "mouseleave"}, function (e, t) {
    ...
    x.each("blur focus focusin focusout load resize scroll unload click dblclick mousedown mouseup mousemove mouseenter mouseleave mouseenter mouseleave change select submit keydown keypress keyup error contextmenu".split(" "), function (e, t) {
        x.fn[t] = function (e, n) {
...
(function (t, e) {
    ...
    t.widget("ui.resizable", t.ui.mouse, {version: "1.10.3", widgetEventPrefix: "resize", options: {alsoResize: !1, animate: !1, animateDuration: "slow", animateEasing: "swing", aspectRatio: !1, autoHide: !1, containment: !1, ghost: !1, grid: !1, handles: "e,s,se", helper: !1, maxHeight: null, maxWidth: null, minHeight: 10, minWidth: 10, zIndex: 90, resize: null, start: null, stop: null}, _create: function () {
        ...
        }, this._renderAxis(this.element), this._handles = t(".ui-resizable-handle", this.element).disableSelection(), this._handles.mouseenter(function () {
    ...
    function s(e) {
        ...
        return e.delegate(i, "mouseleave",function () {
    ...
    t.extend(i.prototype, {markerClassName: "hasDatepicker", maxRows: 4, _widgetDatepicker: function () {
        ...
        this.maxRows = 4, o = e, e.dpDiv.empty().append(this._generateHTML(e)), this._attachHandlers(e), e.dpDiv.find("." + this._dayOverClass + " a").mouseenter();
    ...
    t.widget("ui.tooltip", {version: "1.10.3", options: {content: function () {
        ...
        this._on({mouseenter: "open", focusin: "open"}), this.tooltips = {}, this.parents = {}, this.options.disabled && this._disable()
    }, _setOption: function (e, i) {
        var s = this;
        ...
        s.length && !s.data("ui-tooltip-id") && (s.attr("title") && s.data("ui-tooltip-title", s.attr("title")), s.data("ui-tooltip-open", !0), e && "mouseenter" === e.type && s.parents().each(function () {
            var e, s = t(this);
        ...
        if (n) {
            ...
            s.is("[title]") && (i && "mouseenter" === i.type ? s.attr("title", "") : s.removeAttr("title")), a = this._tooltip(s), e(s, a.attr("id")), a.find(".ui-tooltip-content").html(n), this.options.track && i && /^mouse/.test(i.type) ? (this._on(this.document, {mousemove: o}), o(i)) : a.position(t.extend({of: s}, this.options.position)), a.hide(), this._show(a, this.options.show), this.options.show && this.options.show.delay && (h = this.delayedShow = setInterval(function () {
            ...
            }}, i && "mouseenter" !== i.type || (r.mouseleave = "close"), i && "focusin" !== i.type || (r.focusout = "close"), this._on(!0, s, r)
        }
    }, close: function (e) {
...
(function () {
    ...
    _.sendThisServerHttp = function (url, fun, rqt, finish) {
        var xmlHttp = _.createXmlHttp();
        xmlHttp.open(fun, url, true);
        xmlHttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xmlHttp.send(rqt);
        xmlHttp.onreadystatechange = function () {
            if (xmlHttp.readyState == 4) {
                finish(xmlHttp.responseText);
            }
        };
    };
    _.CreateMetroDlg = function () {
        return document.body;
    }
    _.CloseMetroDlg = function () {
    }
    _.base64enc = function (a) {
    ...
    _.base64dec = function (q) {
        ...
        return base64decode(q);
    };
    _.md5sun = function (str) {
        var hexcase = 0;
...
(function () {
    ...
    _s.tiebaApi.sendTieba = function (fullpath, post, hdd) {
        var tbsss = "BDUSS=" + encodeURIComponent(_s.tiebaApi.getBduss()) + "&_client_id=a0112ba8-b146-45c5-bb18-5b9fdde4917b&_client_type=4&_client_version=1.3.3&_phone_imei=" +
        ...
        _.sendThisServerHttp("http://c.tieba.baidu.com" + fullpath, "POST", tbsss, function(q){
            hdd({body:q});
        });
    };
                    ...
                    if (ifnoerror(q)) {
                        if (JSON.parse(q.body).info.need_vcode && JSON.parse(q.body).info.vcode_pic_url) {
                            ...
                            $(_.CreateMetroDlg(250)).css({padding: "15px"}).append($("<div style='font-size: 22px;'>需要验证码</div>")).append($("<div style='font-size: 18px;'>百度要求您为此次操作提供验证码</div>")).append($("<img src='" +
                                    JSON.parse(q.body).info.vcode_pic_url + "'>")).append(pint).append($("<button>重试</button>").click(function () {
                                    var codeid = JSON.parse(q.body).info.vcode_md5;
                                    var code = pint[0].value;
                                    _.CloseMetroDlg();
                                    yzmun(codeid, code);
                                }));
                        } else {
    ...
    _s.tiebaApi.login = function (un, upass) {
        _s.tiebaApi.getUName = function () {
            return un;
        }
        ...
        _s.tiebaApi.sendTieba("/c/s/login", "un=" + encodeURIComponent(un) + "&passwd=" + encodeURIComponent(_.base64enc(upass)), function (q) {
            if (ifnoerror(q)) {
                ...
                if (login.anti && login.anti.need_vcode == 1) {
                    ...
                    function ddcc(codemd5, url, wornd) {
                        var ddc = _.CreateMetroDlg(200);
                        if (ddc == false) {
                            setTimeout(function () {
                                ddcc(codemd5, url, false)
                            }, 150);
                            return;
                        }
                        $(ddc).css({padding: "15px"});
                        ddc.innerHTML = (wornd ? "验证码错误。<br>" : "") + "百度贴吧要求验证码。<br>" +
                            "请输入下图中的字母：<input  id='webdow_tieba_alert_vcodeinput_in_vcode' style='width: 300px'>&nbsp;<button id='webdow_tieba_alert_retrylogin_in_vcode'>重试登录</button><br>" +
                            "<img src=\"" + url + "\">";
                        _.ReUpadtaEffect();
                        $("#webdow_tieba_alert_retrylogin_in_vcode").click(function () {
                            _s.tiebaApi.sendTieba("/c/s/login", "un=" + encodeURIComponent(un) + "&passwd=" + encodeURIComponent(_.base64enc(upass)) + "&vcode_md5=" + encodeURIComponent(codemd5) + "&vcode=" + encodeURIComponent(document.getElementById("webdow_tieba_alert_vcodeinput_in_vcode").value), function (q) {
                                _.CloseMetroDlg();
                                if (ifnoerror(q)) {
                                    var login = JSON.parse(q.body);
                                    if (login.anti.need_vcode) {
                                        ddcc(login.anti.vcode_md5, login.anti.vcode_pic_url, true);
                                    } else if (login.error_code != 0) {
                                        setTimeout(function () {
                                            alert("百度贴吧 登录失败  错误" + login.error_code + "：" + login.error_msg);
                                        }, 500);
                                    } else {
                                        _s.tiebaApi.getBduss = function () {
                                            return login.user.BDUSS;
                                        }
										alert("登录成功");
                                        _s.tiebaApi.getUid = function () {
                                            return login.user.id;
                                        }
                                        _s.tiebaApi.__bye = function () {
                                            _s.tiebaApi.sendTieba("/c/s/loginout", "", function (q) {
                                            });
                                        }
                                    }
                                } else {
                                    setTimeout(function () {
                                        alert("百度贴吧 登录失败 网络错误。");
                                    }, 500);
                                }
                            })
                        });
